...
function PinnedBadgeSelect({ value, options, onChange, disabled }: { value: string; options: { value: string; label: string; difficulty?: string }[]; onChange: (value: string) => void; disabled: boolean }) {
  const [isOpen, setIsOpen] = useState(false);
  const [query, setQuery] = useState("");
  const selected = options.find((option) => option.value === value) || options[0];
  const matchingOptions = options.filter((option) => `${option.label} ${option.difficulty || ""}`.toLowerCase().includes(query.trim().toLowerCase()));
  const toggleOpen = () => {
    setIsOpen((open) => {
      if (!open) setQuery("");
      return !open;
  ...
  return <div className="relative mt-2"><button type="button" onClick={toggleOpen} disabled={disabled} aria-haspopup="listbox" aria-expanded={isOpen} className={`border-border bg-surface-soft text-font-primary focus:border-accent-cold flex w-full items-center justify-between rounded-lg border px-3 py-2.5 text-left text-sm outline-none disabled:cursor-not-allowed disabled:opacity-50 ${isOpen ? "border-accent-cold" : ""}`}><span className="truncate">{selected.label}</span><FiChevronDown className={`text-font-muted ml-3 h-4 w-4 shrink-0 transition-transform ${isOpen ? "rotate-180" : ""}`} /></button>{isOpen && <div role="listbox" className="border-border bg-surface absolute z-20 mt-1.5 w-full rounded-xl border p-1.5 shadow-black"><label className="border-border bg-surface-soft focus-within:border-accent-cold flex items-center gap-2 rounded-lg border px-2.5 py-2"><FiSearch className="text-font-muted h-4 w-4 shrink-0" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search badges" autoFocus className="text-font-primary placeholder:text-font-muted min-w-0 flex-1 bg-transparent text-sm outline-none" /></label><div className="mt-1 max-h-64 overflow-y-auto pr-0.5">{matchingOptions.length ? matchingOptions.map((option) => <button key={option.value || "none"} type="button" role="option" aria-selected={option.value === value} onClick={() => { onChange(option.value); setIsOpen(false); }} className={`flex w-full items-center justify-between gap-3 rounded-lg px-2.5 py-2 text-left text-sm ${option.value === value ? "bg-brand-tertiary text-font-primary" : "text-font-secondary hover:bg-surface-soft hover:text-font-primary"}`}><span className="min-w-0"><span className="block truncate">{option.label}</span>{option.difficulty && <span className="text-accent-cold mt-0.5 block text-xs">{option.difficulty}</span>}</span>{option.value === value && <FiCheck className="h-4 w-4 shrink-0" />}</button>) : <p className="text-font-muted px-2.5 py-3 text-sm">No matching badges.</p>}</div></div>}</div>;
}
...
function settingsError(reason: unknown, fallback: string, usernameChangedAt?: string | null) {
  ...
  if (normalized.includes("username can only be changed twice per month")) {
    ...
    if (changedAt && !Number.isNaN(changedAt.getTime())) {
      ...
      return `You have used both username changes this month. Try again in ${remainingDays}d.`;
    }
    return "You have used both username changes this month. Try again next month.";
  }
  ...
  if (code === "captcha_failed" || normalized.includes("captcha")) return "Complete the security check and try again.";
...
function VisibilityOption({ checked, disabled, onChange, title, description }: { checked: boolean; disabled: boolean; onChange: (checked: boolean) => void; title: string; description: string }) {
  return <label className={`border-border bg-surface-soft/60 flex items-start gap-3 rounded-lg border p-3 ${disabled ? "cursor-wait opacity-70" : "cursor-pointer"}`}><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} disabled={disabled} className="peer sr-only" /><span className={`border-border bg-surface-soft peer-focus-visible:ring-accent-cold mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded border transition-colors peer-focus-visible:ring-2 ${checked ? "border-accent-cold bg-brand-tertiary text-font-primary" : "hover:border-font-muted"}`}>{checked && <FiCheck className="h-3 w-3" />}</span><span><span className="text-font-primary block text-sm font-medium">{title}</span><span className="text-font-muted mt-0.5 block text-xs leading-relaxed">{description}</span></span></label>;
}
...
function Settings() {
  ...
  const [passwordSetupCaptchaToken, setPasswordSetupCaptchaToken] = useState<string | null>(null);
  const [passwordSetupCaptchaReset, setPasswordSetupCaptchaReset] = useState(0);
  const [passwordChangeCaptchaToken, setPasswordChangeCaptchaToken] = useState<string | null>(null);
  const [passwordChangeCaptchaReset, setPasswordChangeCaptchaReset] = useState(0);
  ...
  const [isHomepageUsernameButtonSaving, setIsHomepageUsernameButtonSaving] = useState(false);
  ...
  const updatePinnedBadgeButtonVisibility = async (hideButton: boolean) => {
    clearFeedback();
    setIsPinnedBadgeButtonSaving(true);
    try {
      await saveUserProfile(user.id, { hide_pinned_badge_edit: hideButton });
      setNotice(hideButton ? "Pinned badge edit button hidden." : "Pinned badge edit button shown.");
    } catch (reason) {
      setError(settingsError(reason, "Pinned badge button visibility could not be updated."));
    } finally {
      setIsPinnedBadgeButtonSaving(false);
    }
  };
  const updateHomepageUsernameButtonVisibility = async (hideButton: boolean) => {
    clearFeedback();
    setIsHomepageUsernameButtonSaving(true);
    try {
      await saveUserProfile(user.id, { hide_homepage_username_edit: hideButton });
      setNotice(hideButton ? "Homepage username edit button hidden." : "Homepage username edit button shown.");
    } catch (reason) {
      setError(settingsError(reason, "Homepage username button visibility could not be updated."));
    } finally {
      setIsHomepageUsernameButtonSaving(false);
    }
  };
  ...
  const setPassword = async () => {
    clearFeedback();
    if (!passwordSetupCaptchaToken) return;
    const { error: resetError } = await supabase.auth.resetPasswordForEmail(user.email || "", {
      redirectTo: `${window.location.origin}/reset-password`,
      captchaToken: passwordSetupCaptchaToken,
    });
    setPasswordSetupCaptchaToken(null);
    setPasswordSetupCaptchaReset((value) => value + 1);
  ...
  const changePassword = async () => {
    ...
    if (!passwordChangeCaptchaToken) return setError("Complete the security check to continue.");
    const { error: reauthError } = await supabase.auth.signInWithPassword({
      email: user.email || "",
      password: currentPassword,
      options: { captchaToken: passwordChangeCaptchaToken },
    });
    setPasswordChangeCaptchaToken(null);
    setPasswordChangeCaptchaReset((value) => value + 1);
    if (reauthError) return setError(settingsError(reauthError, "Your current password is incorrect."));
  ...
  return <section className="flex-1"><div className="min-h-full w-full py-7 sm:py-10 lg:py-12"><div className="mx-auto w-full max-w-5xl px-3 sm:px-7 xl:max-w-7xl">
    ...
    {activeTab === "account" && <div className="space-y-4"><Section title="Email address" description="A confirmation link is sent to the new address before your sign-in email changes."><div className="flex flex-col gap-3 sm:flex-row sm:items-end"><label className="text-font-secondary min-w-0 flex-1 text-sm">Email address<div className="relative mt-2"><FiMail className="text-font-muted pointer-events-none absolute top-1/2 left-3 -translate-y-1/2" /><Input type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="pl-10" /></div></label><button type="button" onClick={changeEmail} className="bg-brand-secondary text-font-primary rounded-lg px-3 py-2.5 text-sm font-medium">Change email</button></div>{pendingEmail && <Notice message={`Waiting for confirmation from ${pendingEmail}.`} />}</Section>
      <Section title="Password recovery" description="Need a password reset email? You can request one without signing out."><button type="button" onClick={() => navigate("/forgot-password")} className="border-border text-font-primary hover:bg-effect-glass rounded-lg border px-3 py-2.5 text-sm font-medium">Reset password by email</button></Section>
