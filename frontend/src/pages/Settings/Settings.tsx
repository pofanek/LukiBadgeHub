...
function Settings() {
  ...
  return <section className="flex-1"><div className="min-h-full w-full py-7 sm:py-10 lg:py-12"><div className="mx-auto w-full max-w-5xl px-3 sm:px-7">
    ...
    {activeTab === "profile" && <div className="space-y-4">
      ...
      <Section title="Pinned badge" description="Choose one badge you have earned to feature on your profile."><div className="text-font-secondary text-sm">Badge<PinnedBadgeSelect value={String(pinnedBadgeId || "")} onChange={(value) => void updatePinnedBadge(value)} disabled={isGamesLoading || isPinSaving} options={[{ value: "", label: "No pinned badge" }, ...ownedBadges.map(({ badge, game }) => ({ value: String(badge.id), label: `${badge.name} — ${game.title}`, difficulty: getBadgeDifficultyLabel(badge.difficulty) }))]} /></div>{!isGamesLoading && !ownedBadges.length && <p className="text-font-muted mt-2 text-xs">Earn a badge to pin it here.</p>}<label className={`border-border bg-surface-soft/60 mt-4 flex items-start gap-3 rounded-lg border p-3 ${isPinnedBadgeButtonSaving ? "cursor-wait opacity-70" : "cursor-pointer"}`}><input type="checkbox" checked={Boolean(profile?.hide_pinned_badge_edit)} onChange={(event) => void updatePinnedBadgeButtonVisibility(event.target.checked)} disabled={isPinnedBadgeButtonSaving} className="peer sr-only" /><span className={`border-border bg-surface-soft peer-focus-visible:ring-accent-cold mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded border transition-colors peer-focus-visible:ring-2 ${profile?.hide_pinned_badge_edit ? "border-accent-cold bg-brand-tertiary text-font-primary" : "hover:border-font-muted"}`}>{profile?.hide_pinned_badge_edit && <FiCheck className="h-3 w-3" />}</span><span><span className="text-font-primary block text-sm font-medium">Hide “Edit pinned badge” button</span><span className="text-font-muted mt-0.5 block text-xs leading-relaxed">You can still change your pinned badge here in Settings.</span></span></label></Section>
